...
( function( $, window, document, undefined ) {
  var pluginName = "filthypillow",
      ...
      defaults = {
        ...
        initialDateTime: moment( ).add(0, 'days'), //function returns moment obj
        enableCalendar: true,
  ...
  FilthyPillow.prototype = {
    ...
    setup: function( ) {
      ...
      if( this.options.enableCalendar )
      {
        ...
        this.clock = new Clock( this.$container.find( ".fp-clock-clock" ),
        {
          minDateTime: this.options.minDateTime,
          maxDateTime: this.options.maxDateTime,
          onChangeTime: $.proxy( function( hour, minute, meridiem ) {
            console.log("here");
            this.updateDateTimeUnit( "hour", hour, false );
            this.updateDateTimeUnit( "minute", minute, false );
            this.updateDateTimeUnit( "meridiem", meridiem, false );
          }, this )
        } );
      }
    ...
    activateSelectorTool: function( step ) {
      ...
      if( this.options.enableCalendar ) {
        if( step === "day" || step === "month" )
        {
          ...
          /**
            Where the calendar is being shown
           */
        else
        {
          this.calendar.hide( );
  ...
  function Clock( $element, options) {
    ...
    // console.log(this.options.onChangeTime);

    var hours = this.date.hours();
    var minutes = this.date.minutes();
    var meridien = "AM";
    ...
    var template = '<div class="fp-cal-container-clock">' +
                      '<div style="margin-bottom:15px" class="fp-cal-nav">' +
                        '<span class="glyphicon glyphicon-chevron-up fp-cal-clock fp-cal-hour-up fp-cal-button"></span>' +
                        '<span class="glyphicon glyphicon-chevron-up fp-cal-clock fp-cal-minute-up fp-cal-button"></span>' +
                        '<span class="glyphicon glyphicon-chevron-up fp-cal-clock fp-cal-meridien-up fp-cal-button"></span>' +
                      '</div>' +
                      '<div style="margin-bottom:15px" class="fp-cal-nav">' +
                        '<span class="fp-cal-clock fp-cal-hour">' + hours + '</span>' +
                        '<span class="fp-cal-clock fp-cal-minute">' + minutes + '</span>' +
                        '<span class="fp-cal-clock fp-cal-meridien">' + meridien + '</span>' +
                      '</div>' +
                      '<div class="fp-cal-nav">' +
                        '<span class="glyphicon glyphicon-chevron-down fp-cal-clock fp-cal-hour-down fp-cal-button"></span>' +
                        '<span class="glyphicon glyphicon-chevron-down fp-cal-clock fp-cal-minute-down fp-cal-button"></span>' +
                        '<span class="glyphicon glyphicon-chevron-down fp-cal-clock fp-cal-meridien-down fp-cal-button"></span>' +
                      '</div>' +
                    '</div>';

    this.$container = $( template );
    this.$hour = this.$container.find( ".fp-cal-hour" );
    this.$minute = this.$container.find( ".fp-cal-minute" );
    this.$meridien = this.$container.find( ".fp-cal-meridien" );
    this.$upHour = this.$container.find( ".fp-cal-hour-up" );
    this.$downHour = this.$container.find( ".fp-cal-hour-down" );
    this.$upMinute = this.$container.find( ".fp-cal-minute-up" );
    this.$downMinute = this.$container.find( ".fp-cal-minute-down" );
    this.$upMeridien = this.$container.find( ".fp-cal-meridien-up" );
    this.$downMeridien = this.$container.find( ".fp-cal-meridien-down" );
  }
  ...
  Clock.prototype.addEvents = function( ) {
    this.$upHour.on( "click", $.proxy( this.nextHour, this ) );
    this.$downHour.on( "click", $.proxy( this.prevHour, this ) );
    this.$upMinute.on( "click", $.proxy( this.nextMinute, this ) );
    this.$downMinute.on( "click", $.proxy( this.prevMinute, this ) );
    this.$upMeridien.on( "click", $.proxy( this.nextMeridien, this ) );
    this.$downMeridien.on( "click", $.proxy( this.prevMeridien, this ) );
  };

  Clock.prototype.removeEvents = function( ) {
    this.$upHour.off( "click" );
    this.$downHour.off( "click" );
    this.$upMinute.off( "click" );
    this.$downMinute.off( "click" );
    this.$upMeridien.off( "click" );
    this.$downMeridien.off( "click" );
  };

  Clock.prototype.buildTemplate = function( ) {
    ...
    this.$meridien.text( meridien );
  };

  Clock.prototype.nextHour = function( ) {
    this.date.add( "hour", 1 );
    this.render( );
    this.onChangeTime();
  }

  Clock.prototype.prevHour = function( ) {
    this.date.subtract( "hour", 1 );
    this.render( );
    this.onChangeTime();
  }

  Clock.prototype.nextMinute = function( ) {
    this.date.add( "minute", 1 );
    this.render( );
    this.onChangeTime();
  }

  Clock.prototype.prevMinute = function( ) {
    this.date.subtract( "minute", 1 );
    this.render( );
    this.onChangeTime();
  }

  Clock.prototype.nextMeridien = function( ) {
    this.date.add( "hour", 12 );
    this.render( );
    this.onChangeTime();
  }

  Clock.prototype.prevMeridien = function( ) {
    this.date.subtract( "hour", 12 );
    this.render( );
    this.onChangeTime();
  }

  Clock.prototype.onChangeTime = function( ) {
    console.log("here Time");
    this.options.onChangeTime( this.date.get( "hour" ), this.date.get( "minute" ), this.$meridien.text());
  };
